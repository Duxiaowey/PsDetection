...
class coco(imdb):
    def __init__(self, image_set, year):
        ...
        # _view_map 字典记录子集数据来源
        self._view_map = {
            'minival2014': 'val2014',  # 5k val2014 subset
            'valminusminival2014': 'val2014',  # val2014 \setminus minival2014
            'test-dev2015': 'test2015',
        }
        # _data_name 为该数据集（coco_name）来源。若为子集则记录源数据集；若为源数据集则记录该数据及名称。
    ...
    #返回字典，字典记录'width'；'height'； 'boxes': 矩阵； 'gt_classes': [记录类别序号]； 
    # 'gt_overlaps': 矩阵,该类为1，其他处为-1； 'flipped': False；'seg_areas': 记录obj区域信息。
