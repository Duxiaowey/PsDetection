...
def proposal_target_layer(rpn_rois, rpn_scores, gt_boxes, _num_classes):
    """
    Return classification labels and bbox regression offsets.

    Parameter
    ---------
    rpn_rois: ndarray
        proposal boxes from proposal_layer, [0, x1, y1, x2, y2]
    rpn_scores: list
        scores from proposal_layer
    gt_boxes: ndarray
        ground truth
    _num_classes: int
        number of classes
    ...
    all_rois = rpn_rois     # (0, x1, y1, x2, y2)
    all_scores = rpn_scores # [num_of_rois, ]
...
def _get_bbox_regression_labels(bbox_target_data, num_classes):
    ...
    -------
    bbox_target: ndarray
        N x 4K blob of regression targets, K为类别数
    bbox_inside_weights: 
        N x 4K blob of weights
    """

    clss = bbox_target_data[:, 0]
    bbox_targets = np.zeros((clss.size, 4 * num_classes), dtype=np.float32)
    bbox_inside_weights = np.zeros(bbox_targets.shape, dtype=np.float32)
    inds = np.where(clss > 0)[0]    # 提取前景类别
    # 将每一类安在应该存在的位置，如：第一类的4个target在4-8之间，第二类在8-12之间，0类也就是背景在1-4之间
    for ind in inds:
        cls = clss[ind]
        start = int(4 * cls)
        end = start + 4
...
def _compute_targets(ex_rois, gt_rois, labels):
    ...
    ------
    : ndarray
        每行对应每个label及偏差值, [label, dx, dy, dw, dh]
    """
...
def _sample_rois(all_rois, all_scores, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
    """
    Return a random sample of RoIs comprising foreground and background examples.

    Return
    ------
    labels: list
        前景=1, 背景=0
    rois, roi_scores: roi区域及得分
    bbox_targets: ndarry
        [dx, dy, dw, dh]
    bbox_inside_weights: ndarray
        输入权重        
